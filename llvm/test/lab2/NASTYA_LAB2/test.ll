...
; COM: Simple doWhile & for loops test

; declare void @loop_start()
; declare void @loop_end()

define dso_local void @_Z14simple_dowhileii(i32 noundef %n, i32 noundef %inv) #0 {
entry:
  %n.addr = alloca i32, align 4
  %inv.addr = alloca i32, align 4
  %res = alloca i32, align 4
...
; CHECK: define dso_local void @_Z14simple_dowhileii(i32 noundef %n, i32 noundef %inv) {
; CHECK: entry:
; CHECK-NEXT:   %n.addr = alloca i32, align 4
; CHECK-NEXT:   %inv.addr = alloca i32, align 4
; CHECK-NEXT:   %res = alloca i32, align 4
...
; CHECK-NEXT:   br label %for.cond
; CHECK-EMPTY: 
; CHECK: for.cond:                                         ; preds = %for.inc, %if.then
; CHECK-NEXT:   %1 = load i32, ptr %i, align 4
; CHECK-NEXT:   %2 = load i32, ptr %n.addr, align 4
; CHECK-NEXT:   %cmp1 = icmp slt i32 %1, %2
...
; CHECK-EMPTY: 
; CHECK: do.body:                                          ; preds = %do.cond, %if.else
; CHECK-NEXT:   %6 = load i32, ptr %inv.addr, align 4
; CHECK-NEXT:   %7 = load i32, ptr %res, align 4
; CHECK-NEXT:   %sub = sub nsw i32 %7, %6
; CHECK-NEXT:   store i32 %sub, ptr %res, align 4
; CHECK-NEXT:   %8 = load i32, ptr %counter, align 4
; CHECK-NEXT:   %dec = add i32 %8, -1
; CHECK-NEXT:   store i32 %dec, ptr %counter, align 4
; CHECK-NEXT:   br label %do.cond
; CHECK-EMPTY: 
; CHECK: do.cond:                                          ; preds = %do.body
; CHECK-NEXT:   %9 = load i32, ptr %counter, align 4
; CHECK-NEXT:   %10 = load i32, ptr %n.addr, align 4
; CHECK-NEXT:   %cmp2 = icmp ugt i32 %9, %10
; CHECK-NEXT:   br i1 %cmp2, label %do.body, label %do.end
; CHECK-EMPTY: 
; CHECK: do.end:                                           ; preds = %do.cond
; CHECK-NEXT:   call void @loop_end()
; CHECK-NEXT:   br label %if.end
; CHECK-EMPTY: 
; CHECK: if.end:                                           ; preds = %do.end, %for.end
; CHECK-NEXT:   ret void
; CHECK-NEXT: }
